...
function parking_get_counts(mysqli $conn) {
    ...
    $counts['available_slots'] = max(
        0,
        $counts['total_slots'] - $counts['occupied_slots'] - $counts['reserved_slots']
    );
...
function parking_create_reservation(mysqli $conn, array $payload) {
    ...
    $checkStmt = $conn->prepare(
        "SELECT COUNT(*) AS total FROM cars WHERE slot_id = ? AND status IN ('reserved', 'occupied')"
    );
    ...
    try {
        $insertCar = $conn->prepare(
            "INSERT INTO cars (plate_number, car_model, car_color, slot_id, parking_guid, status) VALUES (?, ?, ?, ?, ?, 'reserved')"
        );
        ...
        $insertCar->bind_param('sssss', $plate, $model, $color, $slotId, $guid);
        $insertCar->execute();
        $insertCar->close();

        $historyUserColumn = parking_column_exists($conn, 'parking_history', 'user_id');
        $historySlotColumn = parking_history_slot_column($conn);
        $historyAmountColumn = parking_history_amount_column($conn);

        if ($historyUserColumn && $historyAmountColumn) {
            $insertHistory = $conn->prepare(
                "INSERT INTO parking_history (plate_number, {$historySlotColumn}, user_id, entry_time, {$historyAmountColumn}) VALUES (?, ?, ?, NOW(), 0)"
            );
            if ($insertHistory) {
                $insertHistory->bind_param('ssi', $plate, $slotId, $userId);
                $insertHistory->execute();
                $insertHistory->close();
            }
        } elseif ($historyAmountColumn) {
            $insertHistory = $conn->prepare(
                "INSERT INTO parking_history (plate_number, {$historySlotColumn}, entry_time, {$historyAmountColumn}) VALUES (?, ?, NOW(), 0)"
            );
            if ($insertHistory) {
                $insertHistory->bind_param('ss', $plate, $slotId);
                $insertHistory->execute();
                $insertHistory->close();
            }
        }
...
function parking_mark_entry(mysqli $conn, $guid) {
    ...
    $historyStmt = $conn->prepare(
        "UPDATE parking_history SET entry_time = NOW() WHERE plate_number = ? AND {$historySlotColumn} = ? AND exit_time IS NULL ORDER BY id DESC LIMIT 1"
    );
    if ($historyStmt) {
...
function parking_complete_exit(mysqli $conn, $guid) {
    ...
    $historySlotColumn = parking_history_slot_column($conn);
    $historyStmt = $conn->prepare(
        "SELECT * FROM parking_history WHERE plate_number = ? AND {$historySlotColumn} = ? AND exit_time IS NULL ORDER BY id DESC LIMIT 1"
    );
    if (!$historyStmt) {
    ...
    try {
        ...
        $updateHistory = $conn->prepare(
            "UPDATE parking_history SET exit_time = ?, {$historyAmountColumn} = ? WHERE id = ?"
        );
        if (!$updateHistory) {
...
function parking_get_ticket_data(mysqli $conn, $guid) {
    ...
    $historyAmountColumn = parking_history_amount_column($conn);
    $historyStmt = $conn->prepare(
        "SELECT entry_time, exit_time, {$historyAmountColumn} AS cost FROM parking_history WHERE plate_number = ? AND {$historySlotColumn} = ? ORDER BY id DESC LIMIT 1"
    );
...
function parking_get_report_summary(mysqli $conn) {
    $summary = [
        'cars_today' => 0,
        'total_revenue' => 0.0,
        'most_used_slot' => 'N/A',
    ...
    $carsToday = $conn->query("SELECT COUNT(*) AS cnt FROM parking_history WHERE DATE(entry_time) = '{$today}'");
    ...
    $revenue = $conn->query("SELECT COALESCE(SUM({$historyAmountColumn}), 0) AS total FROM parking_history");
    if ($revenue && $row = $revenue->fetch_assoc()) {
        $summary['total_revenue'] = (float) $row['total'];
    }

    $mostUsed = $conn->query("SELECT {$historySlotColumn} AS slot_id, COUNT(*) AS cnt FROM parking_history GROUP BY {$historySlotColumn} ORDER BY cnt DESC LIMIT 1");
    if ($mostUsed && $row = $mostUsed->fetch_assoc()) {
        $summary['most_used_slot'] = $row['slot_id'] ?: 'N/A';
    }

    $totalSessions = $conn->query("SELECT COUNT(*) AS cnt FROM parking_history");
...
function parking_get_recent_history(mysqli $conn, $limit = 20) {
    $limit = max(1, (int) $limit);
    $historySlotColumn = parking_history_slot_column($conn);
    $historyAmountColumn = parking_history_amount_column($conn);
    $result = $conn->query("SELECT id, plate_number, {$historySlotColumn} AS slot_id, entry_time, exit_time, {$historyAmountColumn} AS cost FROM parking_history ORDER BY entry_time DESC LIMIT {$limit}");
